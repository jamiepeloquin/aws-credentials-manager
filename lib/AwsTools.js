...
class AwsTools {
    ...
    async changePassword(oldPassword) {
        ...
        const awsSymbols = "!@#$%^&*()_+-=[]{}|'"
        const pwLength = rules.MinimumPasswordLength >= 10 ? rules.MinimumPasswordLength : 10
        // @TODO: Change to use AWS Secrets Manager to generate the new password.
     ...
     */
    static fetchProfiles(profileName=""){
        profileName = (profileName.toString()).trim()
        this.credentialsPath = process.env.AWS_SHARED_CREDENTIALS_FILE || `${process.env.HOME}/${AWS_HOME_DIR}`;
